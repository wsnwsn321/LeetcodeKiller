    ...
    public List<List<Integer>> verticalOrder(TreeNode root) {
        ...
        while (queue.size() > 0) {
            ...
            for (int i = 0; i < size; ++i) {
                ...
                //put current node into its column list
                if (!level.containsKey(map.get(cur)))
                    level.put(map.get(cur), new ArrayList<>());
                level.get(map.get(cur)).add(cur.val);
                //left child is on the left column of current node, column - 1
                if (cur.left != null) {
                    map.put(cur.left, map.get(cur) - 1);
                    queue.offer(cur.left);
                }
                //right child is on the left column of current node, column + 1
                if (cur.right != null) {
